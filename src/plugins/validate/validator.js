export default{
    message: {
        uname: "请填写真实姓名",
        account: "账号只能是8-12位字母或数字或字母数字组合",
    ...
    methods: {
        ...
        account: function (value, element, param) {
            if (value == null || this.trim(value) == "") return true;
            return /^(\w|\d){8,12}$/.test(value);
        },
        uname: function (value, element, param) {
            if (value == null || this.trim(value) == "") return true;
            return /(^[\u4e00-\u9fa5]{1}[\u4e00-\u9fa5\.·。]{0,8}[\u4e00-\u9fa5]{1}$)|(^[a-zA-Z]{1}[a-zA-Z\s]{0,8}[a-zA-Z]{1}$)/.test(value);
        },
        ...
        // 密码2
        sPassword: function( value, element, param ) {
            if (value == null || this.trim(value) == "") return true;
            var rex = /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{8,18}$/;
            return rex.test(value);
        },
        //邮箱
