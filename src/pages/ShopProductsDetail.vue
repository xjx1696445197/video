    ...
    <div id="productSDetail" class="content_box">
        ...
        <div class='ptop f28'>
            <div class='table h100 txt_center'>
                ...
                <div class='td' v-on:click='click_go("proevalua1")' v-if="pType==4">
                    <div class='inblock dw f0' v-if="pzt=='proevalua1'">
                        <img src='../static/images/shop/dw.png'>
                    </div>
                    <span class="inblock mar_l5">评价</span>
                </div>
                <div class='td' v-on:click='click_go("proevalua2")' v-if="pType==2">
                    <div class='inblock dw f0' v-if="pzt=='proevalua2'">
                        <img src='../static/images/shop/dw.png'>
                    </div>
                    <span class="inblock mar_l5">评价</span>
                </div>
...
<!--                        <img :onerror="errorUserPhoto" :src="sourceUrlShop+item" class="img"/>-->
        ...
        <div v-if="pType==3">
            <div class='all bgfff pad_20 table'>
                <div class='td f28 all'>{{jfBase.productName | strFilter}}</div>
            </div>
            <div class='pad_l20 bgfff pad_r20 pad_b20 table all mar_b20'>
                <div class='td f28 czt'>
                    <span class='inblock mar_r20 pl10'>{{jfBase.expenseCoin}}积分</span>
                    <span class='inblock txt_xx f28 c666'>市场价￥{{jfBase.originalPrice}}</span></div>
                <div class='td col4 f28 c666 txt_right'>
                    <span class='inblock mar_l20'>兑换数量：{{jfBase.allowExchangeNum}}</span></div>
            </div>
        </div>
        <div v-if="pType==4">
            ...
            <div class='all bgfff pad_20 table'>
                <div class='td f28 all'>
                    <p style="display: flex;justify-content: space-between">
                        <span class='mar_b20 inblock' style="width: 80%">{{qgBase.productName | strFilter}}</span>
                        <span class="copy-btn"
                              v-clipboard:copy="qgBase.productName"
                              v-clipboard:success="onCopy"
                              v-clipboard:error="onError"
                              style="color:#e4811d;display: inline-block;width: 2rem;text-align: right;">复制去咨询</span></p>
                    <div class='f28 czt'>{{qgBase.panicTitle| strFilter}}</div>
                </div>
            </div>
            <div class='pad_l20 bgfff pad_r20 pad_b20 table all mar_b20'>
            ...
            <!--已选规格  -->
            <div class='pad_20 bgfff f28 all table mar_b10' v-on:click='openGuige(1)' v-if="qzZt!==2" id="proevalua1">
                <div class='td'>已选：
        ...
        <div v-if="pType==2">
            ...
            <div class='all bgfff pad_20 table'>
                <div class='td f28 all'>
                    <p style="display: flex;justify-content: space-between">
                        <span class='mar_b20 inblock' style="width: 80%">{{tgBase.title | strFilter}}</span>
                        <span class="copy-btn"
                              v-clipboard:copy="tgBase.title"
                              v-clipboard:success="onCopy" v-clipboard:error="onError"
                              style="color:#e4811d;display: inline-block;width: 2rem;text-align: right;">复制去咨询</span></p>
                    <div class='f28 czt'>{{tgBase.introduction | strFilter}}</div>
                </div>
            </div>
            <div class='pad_l20 bgfff pad_r20 pad_b20 table all mar_b20'>
            ...
            <!--已选规格  -->
            <div class='pad_20 bgfff f28 all table mar_b10' v-on:click='openGuige(1)'  v-if="qzZt!==2" id="proevalua2">
                <div class='td'>已选：
                    <span class='czt'>{{yxGuigestr}}</span>
                </div>
                <div class='td icon_xz f0 txt_right'>
        ...
        <router-link :to="{name:'ShopProductEvaluate',params:{pId:pId}}"
                     class='bgfff all table pad_20 f28' style="">
        ...
        <div class='pad_l20 pad_r20 bgfff'>
            ...
            <div class='border_t pad_t10 pad_b20 all' v-for="(item,index) in evaluate.evaluationList" v-if="index<5">
                ...
                <div class='f28 c666'>{{item.showTime | dataFormat('yyyy-MM-dd hh:mm:ss')}}</div>
            </div>
        </div>
        <!--店铺  -->
        ...
        <div class='bgfff pad_b20 detail_cnt' style='min-height:200px;' v-if="xq_zt==0">
            <div v-if="tabIndex==0">
                <!--详情  -->
        ...
        <div class='oprate_bottom all bgfff f20 c666 txt_center'>
            <div class='table'>
                ...
                <div class='td oprate_btn1 f28' style="background: #a7a7a7" v-if="qzZt==2">已结束</div>
                <div class='td oprate_btn1 f28' v-else v-on:click='openGuige'>{{btn_xztxt}}</div>

            </div>
        </div>
        <div class='box_shadow'  v-show="pucistro" v-on:click='openGuige()'></div>
        <div class='box_guige' :class='guigeOprate!=0?"cur":""' v-show="pucistro">

            <div class='close_btn f0' v-on:click='noopenfuige(0)'>
            ...
            <div class='pad_l20 pad_r20 all clear pad_b20 border_b' style="height: 1.35rem">
                ...
                <div class='L font_div pad_l20 pad_t20'>
                    ...
                    <div class='clear'>
                        <div class='L czt f24' v-if="pType==4"><span class="pr10">￥{{qgBase.panicPrice}}</span> | <span class="pl10">{{qgBase.panicCredits}}</span> 积分</div>
                        <div class='L czt f24' v-if="pType==2"><span class="pr10">￥{{tgBase.price}}</span> | <span class="pl10">{{tgBase.credits}}</span> 积分</div>
                ...
                <div v-for="(item,index) in productGuige">
                    ...
                    <div class='clearfix pad_10 border_b'>
                        <div v-for="(itema,indexa) in item.specificationValue" class='p_guige L f24'
                             :class='yxGuigedata[index].tindex==indexa?"cur":""'
            ...
            <div class='oprate_btn1 all txt_center f30 nyBtn'
                 v-on:click='addCart' v-if="buyTrue&postPack>0">确定</div>
            <!--            -->
            ...
            jsonAjax.get(posturl, param, function (result) {
                if (result.returnCode == 1) {
                    if (that.pType == 3) {
                        ...
                        that.pId=result.resultData.productId;
                        that.parId=result.resultData.panicId;
                        // that.postPack=result.resultData.panicNum;
                        that.postPack = 1;
                        that.sku=result.resultData.sku;
                        that.topTitle=result.resultData.productName;
                        //获取商品规格
                        jsonAjax.post(urlUtil.getApiUrl("product_guige"), { productId: that.pId }, function (result) {
                            // alert(1)
                            if (result.returnCode == 1) {
                        ...
                        that.parId=result.resultData.tuanId;
                        ...
                        if (result.returnCode == 1) {
                            ...
                            jsonAjax.post(urlUtil.getApiUrl("product_detail"), { productId: that.pId}, function (result) {
                                if (result.returnCode == 1) {
                                    ...
                                    that.imgUrls_swiper=lunbo;
                                    that.qgBasetwo=result.resultData;
                                    that.pId=result.resultData.productId
                                    that.sku=result.resultData.sku
                                    that.buyTrue=true;
                                    that.postPack=result.resultData.storeNumber;
...
//鼠标释放时候的函数
            ...
            click_go:function(str){
                ...
                // location.replace("/#/shopProductsDetail/"+this.pId);
            },
       ...
       if(this.productGuige==""){
           ...
           if(this.productGuige!="" && this.yxGuigestr == ''){
               ...
               return
                ...
                this.$store.state.placeOrder.data={productId:that.parId,hdId:that.hdId,quality:that.pNum,sku:that.sku, scoreUseCount: score,parentId:that.pId};

                var orderGlobal=JSON.parse(localStorage.getItem("placeOrder"));
                orderGlobal.data={productId:that.parId,hdId:that.hdId,quality:that.pNum,sku:that.sku, scoreUseCount: score,parentId:that.pId};
                localStorage.setItem("placeOrder",JSON.stringify(orderGlobal))

                this.$router.push({name:"ShopOrderConfirm",params:{type:this.pType,goorders:"1"}});
...
<style scoped>
    ...
    .ptop{ position: fixed; left: 0; right: 0;
        top: 0.88rem; height: 0.8rem;
        background: #fff; border-bottom: 1px solid #fefefe; z-index: 10; line-height: 1}
    ...
    .up_load{padding: 10px;
        border-top: 1px solid #fefefe; border-bottom: 1px solid #fefefe; text-align: center; background: #fff;}
