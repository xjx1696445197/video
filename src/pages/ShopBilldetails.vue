    ...
    <div id="my" class="content_box">
        ...
        <div class="detailsbox">
            <div>
                ...
                <img src="../static/images/shop/Success.png" style="width: 1.2rem;height: 1.2rem">
                <p class="play_monery"><span><span v-if="type==1">支付</span><span v-if="type==2">退款</span>成功</span> <span style="color: #e4811d"><span v-if="type==1">-</span><span v-if="type==2">+</span>    {{listdata.amount}} <span v-if="monthlybitwo==0" >元</span><span v-if="monthlybitwo==1" >积分</span></span></p>
            </div>
            <div>
                <div>
                    <span>支付方式：</span>
                ...
                <div>
                    ...
                    <p><span></span><router-link :to="{name:'ShoporderDetail',params:{orderId:orderId}}"><span  style="color: #e4811d">查看订单详情  〉</span></router-link></p>
                </div>
                <div><span>订单编号：</span><span>{{listdata.ordersNo}}</span></div>
                <div><span>创建时间：</span><span>{{listdata.orderTime|formatDate}}</span></div>
                <div v-if="listdata.arrivalTime!==null && type == 2" style="border-top: .2rem solid #F5F5F5">
                    <span>申请退款时间：</span><span>{{listdata.refundTime|formatDate}}</span></div>
                <div v-if="type == 1">
                    <span>支付时间：</span><span>{{listdata.arrivalTime|formatDate}}</span></div>
