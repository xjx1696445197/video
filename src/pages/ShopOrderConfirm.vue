<template>
    <div class="page">
        ...
        <div class="wrapper" style="overflow-y: scroll">
            <router-link :to="{name:'ShopAddressList',params:{aId:'addrselect'}}"
                         class='border_b pad_20 all table bgfff' v-if="!orderAddress">
            ...
            <router-link :to="{name:'ShopAddressList',params:{aId:orderAddress.customerAcceptAddressId}}"
                         class='border_b pad_20 all table bgfff' v-else>
                <div class='addr_w1 f0 td'>
                    <img class='img' src='../static/images/shop/order_confirm_04.png'>
                </div>
                <div class='pad_l20 pad_r20 td'>
                    ...
                    <div class='c999 f28'>
                        {{orderAddress.regionLocationName}}{{orderAddress.cityName}}{{orderAddress.areaCountyName}}{{orderAddress.address}}
                    </div>
                </div>
                <div class='addr_w2 td'>
                    <img src='../static/images/shop/order_confirm_07.png' class='img'>
            ...
            <div v-for="(item,index) in orderData.shopProductList">
                ...
                <div class='pad_20 bgfff table all border_b' v-for="(itema,indexa) in item.productList">
                    ...
                    <div class='td pad_l10'>
                        ...
                        <div class='clearfix'>
                            <div class='L czt pad_t5 f28' :class="type==3?'txt_xx':''">
                                <span class="pr10">￥{{itema.salesPrice}}</span> |
                                <span class="pl10">{{itema.salesCredits}}</span> 积分
                            </div>
                ...
                <div class='pad_20 bgfff border_b txt_right'>
                    ...
                    <span class='inblock czt f32'>￥{{item.xiaoji}} <span class="pl10"
                            v-if="item.jifen != '0.00'">| {{item.jifen}} 积分</span></span>
                </div>
            </div>
            <div class='mar_t10 pad_20 bgfff clear border_b' style="height: 0.8rem">
                ...
                <div class='R f24'>
                    <div class='L mar_l5 f0' v-for="(item,index) in kuaidiData" v-on:click='kuaidiSelect(index)'>
                        <img src="../static/images/shop/checkbox_ed.png" v-if="index==kuaidiIndex"
                             class='ver_mid checkbox' style="display: inline-block;">
                        <img src="../static/images/shop/checkbox.png" v-else class='ver_mid checkbox'
            ...
            <div class='pad_20 bgfff all table border_b'>
                <div class='td'>运费</div>
                <div class='td txt_right c666 f24'>￥{{orderData.finalPostage}} | {{orderData.finalCreditsPostage}} 积分</div>
            </div>
            <div v-if="type==1||type==5">
                <!--<router-link :to="{name:'ShopinvoiceTab',params:{type:invoiceIndex}}"-->
                             <!--class='pad_20 bgfff all border_b table'>-->
                ...
                <router-link :to="{name:'ShopCouponListselect'}" class='pad_20 bgfff all border_b table'
                             v-if="orderCoupList.length>0">
                    <div class='td'>优惠券抵扣</div>
            ...
            <div class='oprate_bottom bgfff border_b f28'>
                <div class='table h100 all pad_l20'>
                    <div class='td w2 txt_right pad_r20' v-if="type!=3">
                        合计：
                        <span class='czt'><span class="pr10">￥{{orderData.finalPayAmount}}</span> | <span class="pl10">{{orderData.finalPayCredits}}</span> 积分</span>
                    </div>
                    <div class='td w2 txt_right pad_r20' v-if="type==3">
                        合计：
    ...
    export default {
        data() {
            return {
                ...
                goorders: this.$route.params.goorders || 1,//订单来源
            }
        },
        /**初始化组件的时候进行数据处理**/
        created: function () {
            if (this.type==undefined){
                ...
                localStorage.setItem("confirmtype",this.type);
            }
            var that = this;
...
//        orderGlobal.data.shoppingCartIds: JSON.stringify(shoppingCartIds);
                ...
                if (that.goorders == 1) {
                    ...
                    var sa = [{
                        nowBuyFlag: that.type,
                        productId: orderGlobal.data.parentId,
                        quality: orderGlobal.data.quality,
                        sku: orderGlobal.data.sku,
                        parentId: orderGlobal.data.productId
                    }];
                    var sss = {
                        buyNowProductBoList: sa, customerDisCountCouponId: 0, scoreUseCount: that.scoreUseCount,
                        customerId: that.$store.state.userinfo.userId
                    };
                    jsonAjax.post(urlUtil.getApiUrl("orderconfirm_imit"), {buyNowGoToOrderPageBoJSON: JSON.stringify(sss)}, function (result) {
                        console.log(result)
...
//            sss.invoiceInfo = that.$store.state.placeOrder.fapiaoData;
                    ...
                    jsonAjax.post(urlUtil.getApiUrl("ordersave_imit"), {buyNowSaveOrderBoJSON: JSON.stringify(sss)}, function (result) {
                        ...
                        if (result.returnCode == 1) {
                            ...
                            that.$router.push({
                                name: 'ShopOrderPay',
                                query: {orderId: oid, orderSn: order_sn, orderType: 1}
                            });
...
//            sss.invoiceInfo = that.$store.state.placeOrder.fapiaoData;
                    ...
                    var sa = [{
                        ...
                        quality: orderGlobal.data.quality,
                        sku: orderGlobal.data.sku,
                        scoreUseCount: orderGlobal.data.scoreUseCount,
                        parentId: orderGlobal.data.hdId
                    }];
                    var sss = {
                        buyNowProductBoList: sa,
                        customerAddressId: that.orderAddress.customerAcceptAddressId,
                        ...
                        if (result.returnCode == 1) {
                            ...
                            if (that.type == 3) {
                                that.$router.push({
                                    ...
                                    params: {orderId: oid, orderSn: order_sn}
                                });
                                return;
                            } else {
                                //isTotalOrdersNo 是否是总订单号，0是子订单 ordersNo，1是总订单 totalOrdersNo
