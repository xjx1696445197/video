    ...
    <div id="addressEdit" class="content_box">
        ...
        <div class='oprate_bottom'>
            <div class='table all h100'>
                <div class='td txt_center oprate_btn1 f28' v-on:click='saveAddress'>
                    保存
                </div>
            </div>
        </div>
        ...
        <div class="picker_bottom f28 txt_center" v-if="pickerShow">
            <div class="table h100">
                <div class="td f28"  v-on:click="openPicker">确定</div>
            </div>
        </div>
    </div>
</template>
                ...
                if(that.aId=='newaddr')
                {
                    ...
                    jsonAjax.post(urlUtil.getApiUrl("address_add"), param, function (result) {
                        ...
                        if (result.returnCode == 1) {
                            ...
                            jsonAjax.post(urlUtil.getApiUrl("address_list"),
                                {customerId: that.$store.state.userinfo.userId}, function (result) {
                                console.log(result)
                                if (result.returnCode == 1) {
                                    that.addresslist=result.resultData;
                                    that.$store.state.placeOrder.address=that.addresslist[param.address];

                                    var orderGlobal=JSON.parse(localStorage.getItem("placeOrder"));
                                    orderGlobal.address=that.addresslist[param.address];
