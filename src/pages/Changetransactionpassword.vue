<template>
    <div class="page">
        ...
        <div style="margin-top: .88rem">
            ...
            <p><span>密码长度为8-18位数字+字母组合</span><span @click="Retrievetransactionpassword">忘记密码</span></p>
        </div>
        <div class="footer">
        ...
        methods:{
            ...
            loginpassword(){
                ...
                if (that.oldpassword==""){
                    ...
                    jsonAjax.post(urlUtil.getApiUrl("updatePassword"), {customerToken:that.token.customerToken,userId:that.userId,password:that.oldpassword,newPassword:that.newpassword,againNewPassword:that.newpasswords,type:2}, function (result) {
                        if (result.returnCode){
                            that.showTips(result.message)
                            setTimeout(() => {that.$router.go(-1);},500)
