        ...
        <div v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="10">
            <div class='pad_l20 pad_r20 bgfff mar_b20' v-for="(item,index) in orderList">
                ...
                <div v-for="(itema,indexa) in item.productList">
                    <router-link :to="{name:'ShoporderDetail',params:{orderId:item.ordersId,type:0,isReturn:item.isReturn}}"
                ...
                >
                    ...
                    <div class='inblock mar_l10 com_obtn' v-if="item.ordersState==4&item.settlementStatus==1&item.isReturn==0"
                    ...
                    <router-link :to="{name:'ShoporderEvaluate',params:{orderId:item.ordersId,type:1}}"
                                 class='inblock mar_l10 com_obtn' v-if="item.ordersState==5&item.settlementStatus==1&item.productList[0].isReturn==0"
                    >
                        去评价
                    </router-link>
...
//              that.orderList[index].ordersState = 9;
            ...
            orderReceivetwo: function (index) {
                ...
                MessageBox.confirm('确认删除订单?').then(action => {
                    jsonAjax.post(urlUtil.getApiUrl("deleteOrders"), {
                        ordersId: order_id,
                    }, function (result) {
                        if (result.returnCode == 1) {
//              that.orderList[index].ordersState = 9;
