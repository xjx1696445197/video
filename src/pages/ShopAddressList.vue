<template>
    <div id="addressList" class="content_box">
        ...
        <div style="margin-top: 0.88rem"></div>
        <div class='pad_20 txt_center' v-if="addresslist.length<1" style="font-size: .28rem;padding-top:0.5rem;">暂无收货地址~</div>
    ...
    export default {
        ...
        data () {
            return {
                ...
                aIdtwo:this.$route.params.aId,
                addtype:""
            }
        },
        /**初始化组件的时候进行数据处理**/
        created: function () {
            if (this.aIdtwo==undefined){
                this.aIdtwo=localStorage.getItem("aIdtwoloc")
            }else {
                localStorage.setItem("aIdtwoloc",this.aIdtwo);
            }
                this.addtype=localStorage.getItem("confirmtype")
            ...
            jsonAjax.post(urlUtil.getApiUrl("address_list"), {customerId: that.$store.state.userinfo.userId}, function (result) {
                ...
                if (result.returnCode == 1) {
                    ...
                    for (var j = 0; j < result.resultData.length; j++) {
                        console.log(result.resultData[j].isSendAddress)
                        if (result.resultData[j].isSendAddress==1) {
                            var orderGlobal=JSON.parse(localStorage.getItem("placeOrder"));
                            orderGlobal.address=result.resultData[j];
        ...
        methods: {
            routerBack: function () {
                ...
                var that = this;
                that.$router.back();
                // if (that.aIdtwo==="addlist"){
                //     that.$router.push({name:"ShopMy"});
                // }else {
                //     that.$router.push({name:"ShopOrderConfirm"});
                // }
            },
            addrSelect:function (index) {
                ...
                jsonAjax.post(urlUtil.getApiUrl("address_setmoren"), { customerAcceptAddressId: list[i].customerAcceptAddressId,
                    ...
                    if (that.aIdtwo==="addlist"){
                        ...
                        that.$router.push({name:"ShopOrderConfirm",params:{type:that.addtype}});
                    }
                    // console.log(result);
                    if (result.returnCode == 1) {
